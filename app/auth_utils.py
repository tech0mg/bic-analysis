...
def login_page():
    st.title("店舗ログイン")
    # DBから店舗名リストを取得してプルダウン表示
    session = SessionLocal()
    try:
        stores = session.query(Store).filter(Store.is_available==True).all()
        store_names = [s.name for s in stores]
    except Exception as e:
        st.error(f"店舗リスト取得中にエラーが発生しました: {e}")
        store_names = []
    finally:
        session.close()
    store_name = st.selectbox("店舗名を選択", store_names)
